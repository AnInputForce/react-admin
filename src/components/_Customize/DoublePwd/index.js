...
class DoublePwd extends React.PureComponent {
  handlePasswordChange = () => {
    const { form, confirmField } = this.props;
    const value = {};
    value[confirmField] = undefined;
    form.setFieldsValue(value);
  };

  handleConfirmPassword = (rule, value, callback) => {
    const { form: { getFieldValue }, pwdField } = this.props;
    if (value && value !== getFieldValue(pwdField)) {
      callback('两次密码输入不一致！');
    }
    callback();
  };
    ...
    return (
      <Form.Item label={confirmLabel} {...formItemLayout} {...rest}>
        {getFieldDecorator(confirmField, {
          initialValue:
            typeof confirmInitialValue === 'function' ? confirmInitialValue() : confirmInitialValue,
          validateFirst: true,
          rules: [
  ...
  createLayout = () => {
    ...
    const min = propMin == null ? 6 : propMin;
    const max = propMax == null ? 16 : propMax;
    const width = propWidth == null ? '100%' : propWidth;
    const pwdFormItemLayout = getFormItemLayout(columnLayout, columnIndex);
    const confirmFormItemLayout = getFormItemLayout(columnLayout, columnIndex + 1);
