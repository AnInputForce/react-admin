...
import { digitUppercase } from '../_utils/utils';
...
class AmtInput extends React.PureComponent {
  ...
  componentWillMount() {
    ...
    if (!isNaN(parseInt(value, 10))) {
      const upper = digitUppercase(value);
  ...
  validateAmt = (rule, value, callback) => {
    ...
    const min = propMin == null ? 0 : propMin;
    ...
    if (parseInt(value, 10) > max) {
      const upper = digitUppercase(max);
    ...
    if (parseInt(value, 10) < min) {
      const upper = digitUppercase(min);
    ...
    return (
      ...
      >
        {getFieldDecorator(field, {
          ...
          rules: [{ validator: this.validateAmt }],
        })(
          <Input
