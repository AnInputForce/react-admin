...
class BasicOneOutput extends PureComponent {
  render() {
    const { dispatch, routeid, basicOne } = this.props;
    const { response, cityValues } = basicOne[routeid];
      ...
      const actions = (
        <div>
          ...
          <Button
            type="primary"
            onClick={() => {
              dispatch({ type: 'basic-one/redo', routeid });
            }}
          >
            再做一笔
          </Button>
    ...
    const actions = (
      <div>
        <Button
          type="primary"
          onClick={() => {
            dispatch({ type: 'basic-one/back2edit' });
          }}
        >
            返回修改
        </Button>
...
export default connect(mapStateToProps)(BasicOneOutput);
