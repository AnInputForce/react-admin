...
class BasicTwoOutput extends PureComponent {
  render() {
    const { dispatch, routeid, basicTwo } = this.props;
    const { response, cityValues } = basicTwo[routeid];
      ...
      const actions = (
        <div>
          ...
          <Button
            type="primary"
            onClick={() => {
              dispatch({ type: 'basic-two/redo', routeid });
            }}
          >
            再做一笔
          </Button>
...
export default connect(mapStateToProps)(BasicTwoOutput);
