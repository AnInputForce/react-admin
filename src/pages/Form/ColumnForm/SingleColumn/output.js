...
class SingleColumnOutput extends PureComponent {
  render() {
    const { dispatch, routeid, singleColumn } = this.props;
    const { response, cityValues } = singleColumn[routeid];
      ...
      const actions = (
        <div>
          ...
          <Button
            type="primary"
            onClick={() => {
              dispatch({ type: 'single-column/redo', routeid });
            }}
          >
            再做一笔
          </Button>
    ...
    const actions = (
      <div>
        <Button
          type="primary"
          onClick={() => {
            dispatch({ type: 'single-column/back2edit' });
          }}
        >
            返回修改
        </Button>
