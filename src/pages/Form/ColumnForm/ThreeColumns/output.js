...
class ThreeColumnsInput extends PureComponent {
  render() {
    const { dispatch, routeid, threeColumns } = this.props;
    const { response, cityValues } = threeColumns[routeid];
      ...
      const actions = (
        <div>
          ...
          <Button
            type="primary"
            onClick={() => {
              dispatch({ type: 'three-columns/redo', routeid });
            }}
          >
            再做一笔
          </Button>
    ...
    const actions = (
      <div>
        <Button
          type="primary"
          onClick={() => {
            dispatch({ type: 'three-columns/back2edit' });
          }}
        >
            返回修改
        </Button>
...
export default connect(mapStateToProps)(ThreeColumnsInput);
