...
class DetailComp extends PureComponent {
  handleGo2PreviousPage = () => {
    const { dispatch, gobackRoute } = this.props;
      ...
      <Card bordered={false} className={styles.card}>
        <Row>
          <h1 className={styles.firstTitle}>
            {`${data.title} 组件`}
            <Tag className={styles.tag} color="#2db7f5">
              {data.version}
            </Tag>
          </h1>
        </Row>
        ...
        <Row>
          <div className={styles.api}>
            <pre>
              <code className="error-mapped-context typescript">{data.api}</code>
            </pre>
          </div>
        </Row>
        <Row>
...
function mapStateToProps(state) {
  return {
    data: state['detail-comp'].data,
    gobackRoute: state['detail-comp'].gobackRoute,
  };
}
